...
document.getElementById("submitPlayers").onclick = () => {
    ...
    if (continueGame) {
        ...
        const choiceScroll = (array, inclusive) => {
            return `<div style="display: flex; align-items: center; height: ${choiceScrollHeight}; max-width: 100%; overflow-x: auto;">${array.reduce((prev, curr) => {return prev + `${inclusive ? "AND" : "OR"}<img src="${curr.img.src}">`;}, "").substring(2)}</div>`;
        };
        ...
        const invulnerableVoldemort = () => {
            if (!inactiveVillains.length) {
                if ((activeGame === "Game 5" || activeGame === "Box 1" || activeGame === "Pack 1") && activeVillains[0] !== lordVoldemort1) {
                    return lordVoldemort1;
                }
                else if ((activeGame === "Game 6" || activeGame === "Box 2" || activeGame === "Pack 2") && activeVillains[0] !== lordVoldemort2) {
                    return lordVoldemort2;
                }
                else if ((activeGame === "Game 7" || activeGame === "Box 3" || activeGame === "Pack 3") && activeVillains[0] !== lordVoldemort3) {
                    return lordVoldemort3;
                }
                else if (activeGame === "Box 4" && activeVillains[0] !== lordVoldemort4) {
                    return lordVoldemort4;
                }
                else if (activeGame === "Pack 4" && activeVillains[0] !== lordVoldemort5) {
                    return lordVoldemort5;
                }
            }
            return null;
        };
        ...
        class Player {
            ...
            forcedDiscardAt(index, villainOrDAE) {
                ...
                // Arabella Figg effect
                else if (this.hand[index] === arabellaFigg) {
                    if (!activeMermaid()) this.influence++;
                    this.health++;
                }
                // Eagle Owl effect
                else if (this.hand[index] === eagleOwl) {
                    activeLocation.removeFromLocation();
                }
                // School Owl effect
                else if (this.hand[index] === schoolOwl1 || this.hand[index] === schoolOwl2) {
                    let remainingPlayers = players.filter(player => {return !player.petrified;});
                    if (remainingPlayers.length > 2) {
                        addPlayerChoice("Draw 1 for:", () => {return remainingPlayers.length;}, 2, () => {
                            for (let i = 0; i < remainingPlayers.length; i++) {
                                document.getElementsByClassName("choice")[i].appendChild(remainingPlayers[i].heroImage.cloneNode());
                                document.getElementsByClassName("choice")[i].onclick = () => {
                                    remainingPlayers[i].drawCards(1);
                                    remainingPlayers.splice(i, 1);
                                };
                            }
                        });
                    }
                    else remainingPlayers.forEach(player => {player.drawCards(1);});
                }
            ...
            stun() {
                if (!this.stunned) {
                    ...
                    if (getStagSpells().length) {
                        addPlayerChoice("Choose 1:", () => {return 2;}, 1, () => {
                            if (getStagSpells().length) {
                                document.getElementsByClassName("choice")[0].innerHTML = `<p>Harry Potter loses:</p>${choiceScroll(getStagSpells(), false)}<div class="choiceContainer" style="height: calc(100% - 6em - ${choiceScrollHeight})">${healthToken + healthToken}</div><p>Health: ${stagPlayer.health}</p>`;
                                document.getElementsByClassName("choice")[0].onclick = () => {
        ...
        const tergeo1 = new Card("Tergeo", "Box 1", "spell", 2, affectedPlayer => {
            ...
            if (affectedPlayer.hand.length) {
                addPlayerChoice("Do you want to banish a card?", () => {return 2;}, 1, () => {
                    document.getElementsByClassName("choice")[0].innerHTML = choiceScroll(affectedPlayer.hand, false); 
                    document.getElementsByClassName("choice")[0].onclick = () => {
        ...
        const depulso1 = new Card("Depulso", "Box 2", "spell", 3, affectedPlayer => {
            ...
            if (handItems.length || discardItems.length) {
                addPlayerChoice("Choose 1:", () => {return 2;}, 1, () => {
                    ...
                    document.getElementsByClassName("choice")[1].innerHTML = `<p>Banish:</p>${choiceScroll(handItems.concat(discardItems), false)}`;
                    document.getElementsByClassName("choice")[1].onclick = () => {
            ...
            if (playersWithCards.length) {
                addPlayerChoice("Choose a player to banish 1:", () => {return playersWithCards.length + 1;}, 1, () => {
                    for (let i = 0; i < playersWithCards.length; i++) {
                        document.getElementsByClassName("choice")[i].innerHTML = `<p>${playersWithCards[i].hero}:</p>${choiceScroll(playersWithCards[i].hand.concat(playersWithCards[i].discard), false)}`;
                        document.getElementsByClassName("choice")[i].onclick = () => {
        ...
        // Pack 2
        const arabellaFigg = new Card("Arabella Figg", "Pack 2", "ally", 2, () => {
            if (!activeMermaid()) {
                addPlayerChoice("Give 1 influence to:", affectedPlayer => {return players.length;}, 1, () => {
                    for (let i = 0; i < players.length; i++) {
                        document.getElementsByClassName("choice")[i].appendChild(players[i].heroImage.cloneNode());
                        document.getElementsByClassName("choice")[i].innerHTML += `<p>Influence: ${players[i].influence}</p>`;
                        document.getElementsByClassName("choice")[i].onclick = () => {players[i].influence++;};
        ...
        const confringo1 = new Card("Confringo", "Pack 2", "spell", 3, affectedPlayer => {
            affectedPlayer.attack++;
            if (affectedPlayer.hand.length) {
                addPlayerChoice("Banish:", () => {return affectedPlayer.hand.length + 1;}, 1, () => {
                    for (let i = 0; i < affectedPlayer.hand.length; i++) {
                        document.getElementsByClassName("choice")[i].innerHTML = `<img src="${affectedPlayer.hand[i].img.src}">`;
                        document.getElementsByClassName("choice")[i].onclick = () => {affectedPlayer.banishAt(i)};
                    }
                    document.getElementById("choice")[affectedPlayer.hand.length].innerHTML = "<p>Nothing</p>";
                });
            }
        }, false, false);
        const confringo2 = confringo1.clone();
        const eagleOwl = new Card("Eagle Owl", "Pack 2", "ally", 4, affectedPlayer => {
            if (!activeMermaid()) {
        ...
        const evanesco1 = new Card("Evanesco", "Pack 2", "spell", 5, affectedPlayer => {
            let remainingPlayers = [...players];
            const banishChoose = player => {
                if (player.hand.length) {
                    addPlayerChoice(`Banish for ${player.hero}:`, () => {return player.hand.length + 1;}, 1, () => {
                        for (let i = 0; i < player.hand.length; i++) {
                            document.getElementsByClassName("choice")[i].innerHTML = `<img src="${player.hand[i].img.src}">`;
            ...
            if (players.length > 2) {
                addPlayerChoice("Banish 1 for:", () => {return remainingPlayers.length + 1;}, 2, () => {
                    for (let i = 0; i < remainingPlayers.length; i++) {
                        document.getElementsByClassName("choice")[i].innerHTML = `<p>${remainingPlayers[i].hero}:</p>${choiceScroll(remainingPlayers[i].hand, false)}`;
                        document.getElementsByClassName("choice")[i].onclick = () => {
                            banishChoose(remainingPlayers[i]);
                            remainingPlayers.splice(i, 1);
                        };
                    }
                    document.getElementsByClassName("choice")[remainingPlayers.length].innerHTML = "<p>Nevermind</p>";
                    document.getElementsByClassName("choice")[remainingPlayers.length].onclick = () => {remainingPlayers = [];};
                });
            }
            else players.forEach(player => {banishChoose(player);});
        }, false, false);
        const evanesco2 = evanesco1.clone();
        const evanesco3 = evanesco1.clone();
        const percyWeasley = new Card("Percy Weasley", "Pack 2", "ally", 3, affectedPlayer => {}, true, false);
        const schoolOwl1 = new Card("School Owl", "Pack 2", "ally", 3, affectedPlayer => {affectedPlayer.attack++;}, false, false);
        const schoolOwl2 = schoolOwl1.clone();
        ...
        if (activeGame.includes("Box") || activeGame.includes("Pack")) {
            ...
            if (activeGame.includes("Pack")) {
                ...
                if (activeGame !== "Pack 1") {
                    hogwartsCards.push(arabellaFigg, confringo1, confringo2, eagleOwl, evanesco1, evanesco2, evanesco3, percyWeasley);
            ...
            getNeighbors(players[0]).concat([players[0]]).filter(player => {return player.health > 0 && player.hand.filter(card => {return card.type === "item";}).length;}).forEach(player => {
                ...
                if (items().length) {
                    addPlayerChoice("Lose:", () => {return 2;}, 1, () => {
                        ...
                        document.getElementsByClassName("choice")[1].innerHTML = choiceScroll(items(), true);
                        document.getElementsByClassName("choice")[1].onclick = () => {
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            ...
                            if (!activeVillains.filter(villain => {return villain.health || villain.influence}).length && !inactiveVillains.length) {
                                activeVillains = [];
                                // Voldemort
                                if ((activeGame === "Game 5" || activeGame === "Box 1" || activeGame === "Pack 1") && this !== lordVoldemort1) {
                                    activeVillains.push(lordVoldemort1);
                                }
                                else if ((activeGame === "Game 6" || activeGame === "Box 2" || activeGame === "Pack 2") && this !== lordVoldemort2) {
                                    activeVillains.push(lordVoldemort2);
                                }
                                else if ((activeGame === "Game 7" || activeGame === "Box 3" || activeGame === "Pack 3") && this !== lordVoldemort3) {
                                    activeVillains.push(lordVoldemort3);
                                }
                                else if (activeGame === "Box 4" && this !== lordVoldemort4) {
                                    activeVillains.push(lordVoldemort4);
                                }
                                else if (activeGame === "Pack 4" && this !== lordVoldemort5) {
                                    activeVillains.push(lordVoldemort5);
                                }
                                else alert("Victory!");
                                populateVillains();
        ...
        const tomRiddle = new Villain("Tom Riddle", "Game 2", "villain", 6, 0, () => {
            ...
            const tomRiddleEffect = () => {
                addPlayerChoice("Lose:", () => {allies = allies.filter(card => {return players[0].hand.includes(card);}); if (allies.length) return 2; return 0;}, 1, () => {
                    ...
                    document.getElementsByClassName("choice")[1].innerHTML = choiceScroll(players[0].hand, false); 
                    document.getElementsByClassName("choice")[1].onclick = () => {
            ...
            players.forEach(player => {
                ...
                if (allies.length && canHeal(player)) {
                    addPlayerChoice(`Choose for ${player.hero}:`, () => {return 2;}, 1, () => {
                        ...
                        document.getElementsByClassName("choice")[1].innerHTML = choiceScroll(allies, false); 
        ...
        const norbert = new Villain("Norbert", "Box 1", "creature", 0, 6, () => {players[0].health -= 1 + players[0].hand.filter(card => {return card.name === "Detention";}).length;}, () => {players.forEach(player => {
            addPlayerChoice(`Banish for ${player.hero}:`, () => {return 2;}, 1, () => {
                document.getElementsByClassName("choice")[0].innerHTML = choiceScroll(player.hand.concat(player.discard), false);
                document.getElementsByClassName("choice")[0].onclick = () => {
            ...
            players.forEach(player => {
                ...
                if (handItems.length || discardItems.length) {
                    addPlayerChoice("Banish:", () => {return 2;}, 1, () => {
                        document.getElementsByClassName("choice")[0].innerHTML = choiceScroll(handItems.concat(discardItems), false);
                        document.getElementsByClassName("choice")[0].onclick = () => {
                            if (handItems.length + discardItems.length > 1) {
            ...
            if (spells().length) {
                addPlayerChoice("Lose:", () => {return 2;}, 1, () => {
                    if (spells().length && players[0].health > 0) {
                        document.getElementsByClassName("choice")[0].innerHTML = `<div class="choiceContainer">${healthToken + healthToken}</div>`;
                        document.getElementsByClassName("choice")[0].onclick = () => {players[0].health -= 2;};
                        document.getElementsByClassName("choice")[1].innerHTML = `<p>Discard:</p>${choiceScroll(spells(), false)}`;
                        document.getElementsByClassName("choice")[1].onclick = () => {
                            if (spells().length > 1) {
        ...
        const horcrux2 = new Encounter("Horcrux 2", "Game 7", ["attack", "influence"], () => {}, () => {
            if (players[0].hand.length >= 2 && (!activeVillains.includes(bartyCrouchJr) || bartyCrouchJr.petrifiedBy || bartyCrouchJr.health <= 0) && (activeLocation.number > 1 || activeLocation.added)) {
                if (players[0].hand.length > 2) {
                    addPlayerChoice("Discard:", () => {return players[0].hand.length;}, 2, () => {for (let i = 0; i < players[0].hand.length; i++) {document.getElementsByClassName("choice")[i].innerHTML = `<img src="${players[0].hand[i].img.src}">`; document.getElementsByClassName("choice")[i].onclick = () => {players[0].forcedDiscardAt(i, false);};}});
                } 
                else {
        ...
        const fullMoonRises = new Encounter("Full Moon Rises", "Box 2", [], () => {}, () => {
            players.forEach(player => {
                ...
                if (banishable.length) {
                    addPlayerChoice(`Choose 1 for ${player.hero}:`, () => {return 2;}, 1, () => {
                        document.getElementsByClassName("choice")[0].innerHTML = `<p>Banish:</p>${choiceScroll(banishable, false)}`;
                        document.getElementsByClassName("choice")[0].onclick = () => {
                            ...
                            if (inactiveVillains.length) {
                                ...
                                if (!inactiveVillains.length) {
                                    ...
                                    if ((activeGame === "Game 5" || activeGame === "Box 1" || activeGame === "Pack 1") && activeVillains[0] !== lordVoldemort1) {
                                        ...
                                        document.getElementById("villainDraw").appendChild(lordVoldemort5.img);
                                    }
                                }
                            }
                            ...
                            if (players[0].health > 7) players[0].influence += 2;
                                ...
                                playerChoices.unshift(new PlayerChoice("Banish from:", () => {return banishable.length + 1;}, 1, () => {
                                    for (let i = 0; i < banishable.length; i++) {
                                        document.getElementsByClassName("choice")[i].innerHTML = choiceScroll(banishable[i].discard, false);
                                        document.getElementsByClassName("choice")[i].onclick = () => {
                                        ...
                                        setTimeout(() => {
                                            ...
                                            if (!nonPassiveVillains.length || i === nonPassiveVillains.length - 1) {
                                                ...
                                                setTimeout(() => {
                                                    ...
                                                    // Percy Weasley effect
                                                    if (players[0].hand.includes(percyWeasley)) {
                                                        if (players[0].hand.length === 5) players[0].influence += 3;
                                                        else players[0].influence++;
                                                        darken(percyWeasley.img);
                                                    }
